...
from config import RING_SIZE_POW, DEBUG_MODE, SINGLE_NODE
from helper import str_to_bool, write_debug_file
...
if __name__ == "__main__":
    ...
    using_threads = str_to_bool(sys.argv[3])
    ...
    formatted_list = [f"{node.ip}:{node.port}" for node in nodes]
    formatted_list = str(formatted_list).replace("'", '"')
    print(f"'{formatted_list}'")
    write_debug_file(formatted_list, cur_dir, 'nodes.txt', 'w', f=True)
    write_debug_file("", cur_dir, 'debug.log', 'w', f=True)
    ...
    if DEBUG_MODE:
        node_list = []
        for node in nodes:
            node_list.append(f"{node.ip}:{node.port}")
