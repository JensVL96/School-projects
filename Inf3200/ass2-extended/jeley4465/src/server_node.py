...
def stabilize():
    ...
    filename = "stabilize.log"

    # write_debug_file(f"Stabilizing node {host}. its succ is {successor}", cur_dir, filename, f=True)
    
    if successor == host:
        return
    
    x = requests.get(f"http://{successor}:{port}/predecessor")


    if x.status_code != 200:
        # write_debug_file(f"Node {host} could not get predecessor from successor {successor}", cur_dir, filename, f=True)
        for i in range(0, ring_pow-1, ):
            # write_debug_file(f"Node {host} trying to get predecessor from finger {fingerTable[i][1]}", cur_dir, filename, f=True)
            write_debug_file(f"[Stabilize] Node {host} trying to get predecessor from finger {fingerTable[i][1]}", cur_dir, filename, f=True)
            x = requests.get(f"http://{fingerTable[i][1]}:{port}/predecessor")
            if x.status_code == 200:
                # write_debug_file(f"Node {host} got predecessor {x.text} from finger {fingerTable[i][1]}", cur_dir, filename, f=True)
                successor = fingerTable[i][1]
                break

    x = x.text
    # write_debug_file(f"I want to set x: {x}", cur_dir, filename, f=True)
    # if in range
    if x is not None and x != "None":
        if is_in_range(hash_mod(x, ring_size), idx+1, hash_mod(successor, ring_size)-1, ring_size):
            successor = x
            write_debug_file(f"[Stabilize] Node {host} updated its successor to {successor}", cur_dir, filename, f=True)
...
def fix_fingers():
    ...
    try:
        if USE_FINGER_TABLE:
            r = requests.get(f"http://{successor}:{port}/find_successor_finger/{fingerTable[next_finger-1][0]}")
            if r.status_code == 200:
                fingerTable[next_finger][1] = r.text.text
        else:
            r = requests.get(f"http://{successor}:{port}/find_successor/{fingerTable[next_finger-1][0]}")
            if r.status_code == 200:
                fingerTable[next_finger][1] = r.text.text
    except Exception as e:
        # May need more time to recover from a crash
        ...
        if self.path == '/helloworld':
            ...
            try:
                ...
                # Check if the key is stored in the successor node
                    ...
                    message = "None"
                    if forward_node == None or forward_node == host or forward_node == "" or forward_node == "None":
                        pass
                    else:
                            retries += 1
                            if retries <= max_retries: 
                                try:
                                    ...
                                    write_debug_file(f"Error in node {host}, forward_node: {forward_node}, Error: {e}", cur_dir, f=True)
                                    # May need more time to recover from a crash
                                    self.send_response(200)
                                    self.send_header("Content-type", "text/plain")
                                    self.end_headers()
                ...
                write_debug_file(f"Error in find_successor_finger: {e}", cur_dir, f=True)
    ...
    def do_POST(self):
        ...
        if self.path.startswith('/join?nprime='):
            ...
            try:
                ...
                write_debug_file(f"Error in join /find_successor: {e}", cur_dir, f=True)
                # May need more time to recover from a crash
                pass
            ...
            try:
                ...
                write_debug_file(f"Error in notify: {e}", cur_dir, f=True)
