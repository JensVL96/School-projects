import argparse
import http.client
import json
import random
import textwrap
import uuid
import time
import os
import statistics  # For calculating standard deviation
import matplotlib.pyplot as plt  # For plotting
import numpy as np  # For numerical operations

def write_debug_file(message, cur_dir, filname='debug.log', write_mode='a'):
    try:
        # Create the full file path by joining the current directory with the filename
        file_path = os.path.join(cur_dir, filname)
        
        # Open the file in write mode using the full file path
        with open(file_path, write_mode) as file:
            file.write(message + '\n')  # Overwrite the file with the message
    except Exception as e:
        print(f"Error writing to debug file: {e}")


def arg_parser():
    parser = argparse.ArgumentParser(prog="client", description="DHT client")

    parser.add_argument("nodes", type=str, nargs="+",
            help="addresses (host:port) of nodes to test")

    return parser  


class Lorem(object):
    """ Generates lorem ipsum placeholder text"""

    sample = """
        Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod
        tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim
        veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea
        commodo consequat. Duis aute irure dolor in reprehenderit in voluptate
        velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat
        cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id
        est laborum.
        """

    def __init__(self):
        # Lowercase words and strip leading/trailing whitespace
        s = self.sample.lower().strip()
        # Filter out punctuation and other non-alpha non-space characters
        s = filter(lambda c: c.isalpha() or c.isspace(), s)
        # Collect filtered letters back into a string, then split into words
        s = ''.join(s).split()
        # Collapse into a set to dedupe words, then turn back into a list
        self.word_list = sorted(list(set(s)))

        self.min_words = 5
        self.max_words = 20

        self.min_sentences = 3
        self.max_sentences = 6

        self.min_paras = 1
        self.max_paras = 5

    def sentence(self):
        nwords = random.randrange(self.min_words, self.max_words)
        rand_words = [random.choice(self.word_list) for _ in range(0, nwords)]
        rand_words[0] = rand_words[0].capitalize()
        return " ".join(rand_words) + "."

    def paragraph(self):
        nsens = random.randrange(self.min_sentences, self.max_sentences)
        rand_sens = [self.sentence() for _ in range(0, nsens)]
        return textwrap.fill(" ".join(rand_sens))

    def text(self):
        nparas = random.randrange(self.min_paras, self.max_paras)
        rand_paras = [self.paragraph() for _ in range(0, nparas)]
        return "\n\n".join(rand_paras)


lorem = Lorem()


def generate_pairs(count):
    pairs = {}
    for x in range(0, count):
        key = str(uuid.uuid4())
        value = lorem.text()
        pairs[key] = value
    return pairs


def put_value(node, key, value):
    conn = None
    try:
        conn = http.client.HTTPConnection(node)
        conn.request("PUT", "/storage/"+key, value)
        resp = conn.getresponse()
        status = resp.status
        resp.read()  # Ensure the response is read to free the connection
        return status == 200 or status == 201
    except Exception as e:
        print(f"PUT to {node}: EXCEPTION DURING PUT: {e}")
        return False
    finally:
        if conn:
            conn.close()


def get_value_raw(node, key):
    conn = None
    try:
        # Make request
        conn = http.client.HTTPConnection(node)
        conn.request("GET", "/storage/"+key)
        resp = conn.getresponse()
        status = resp.status
        headers = resp.getheaders()
        value = resp.read()

        # Extract headers
        contenttype = "text/plain"
        for h, hv in headers:
            if h.lower() == "content-type":
                contenttype = hv

        # Decode value, if text
        if contenttype == "text/plain":
            value = value.decode("utf-8")
        elif contenttype.startswith("text/plain"):
            value = value.decode("utf-8")

        return status, value, contenttype
    except Exception as e:
        print(f"GET to {node}: EXCEPTION DURING GET: {e}")
        return None, None, None
    finally:
        if conn:
            conn.close()


def get_neighbours(node):
    conn = None
    try:
        conn = http.client.HTTPConnection(node)
        conn.request("GET", "/network")
        resp = conn.getresponse()
        if resp.status != 200:
            neighbors = []
        else:
            body = resp.read()
            neighbors = json.loads(body)
    except Exception as e:
        print(f"Error fetching neighbors from {node}: {e}")
        neighbors = []
    finally:
        if conn:
            conn.close()
    return neighbors


def walk_neighbours(start_nodes):
    to_visit = start_nodes.copy()
    visited = set()
    while to_visit:
        next_node = to_visit.pop()
        if next_node in visited:
            continue
        visited.add(next_node)
        neighbors = get_neighbours(next_node)
        for neighbor in neighbors:
            if neighbor not in visited and neighbor not in to_visit:
                to_visit.append(neighbor)
    return visited


def retrieve_metrics_throughput(nodes, duration=1, N=5):
    """
    Perform N trials of PUT and GET operations within a specified duration (in seconds)
    and collect throughput metrics (number of operations per second).
    Returns lists of PUT counts, GET counts
    """
    pairs = generate_pairs(100)  # Pre-generate a set of key-value pairs

    put_counts = []
    get_counts = []
    get_success_counts = []

    for run in range(1, N+1):
        print(f"Trial {run}/{N}")

        # PUT Throughput Measurement
        put_success = 0
        put_start_time = time.time()
        while time.time() - put_start_time < duration:
            key, value = random.choice(list(pairs.items()))
            put_node = random.choice(nodes)
            success = put_value(put_node, key, value)
            if success:
                put_success += 1
        put_counts.append(put_success)

        # GET Throughput Measurement
        get_success = 0
        get_total = 0
        get_start_time = time.time()
        while time.time() - get_start_time < duration:
            key, expected_value = random.choice(list(pairs.items()))
            get_node = random.choice(nodes)
            status, returned, contenttype = get_value_raw(get_node, key)
            get_total += 1
            if status and status in range(200, 300) and returned == expected_value:
                get_success += 1
        get_counts.append(get_total)
        get_success_counts.append(get_success)

    return put_counts, get_counts, get_success_counts


def retrieve_from_different_nodes_throughput(nodes, duration=1, N=5):
    """
    Retrieve throughput metrics for a given set of nodes.
    Returns average and standard deviation for PUT throughput, GET throughput, and GET success rate.
    """
    print("Retrieving throughput metrics ...")

    put_counts, get_counts, get_success_counts = retrieve_metrics_throughput(nodes, duration, N)

    # Calculate statistics
    avg_put = statistics.mean(put_counts)
    std_put = statistics.stdev(put_counts) if N > 1 else 0

    avg_get = statistics.mean(get_counts)
    std_get = statistics.stdev(get_counts) if N > 1 else 0

    avg_success_rate = (statistics.mean(get_success_counts) / statistics.mean(get_counts)) * 100 if statistics.mean(get_counts) > 0 else 0

    cur_dir = os.getcwd()

    write_debug_file(f"Average PUT requests: {avg_put:.2f} ± {std_put:.2f} requests/sec", cur_dir, "throughput.log")
    write_debug_file(f"Average GET requests: {avg_get:.2f} ± {std_get:.2f} requests/sec", cur_dir, "throughput.log")

    return {
        "put_mean": avg_put,
        "put_std": std_put,
        "get_mean": avg_get,
        "get_std": std_get,
        "success_mean": avg_success_rate,
    }

def main(args):
    # Prepare to store results
    results = {
        "nodes": [],
        "put_mean": [],
        "put_std": [],
        "get_mean": [],
        "get_std": [],
        "success_mean": [],
        "success_std": []
    }

    # Discover all reachable nodes starting from the provided nodes
    nodes = set(args.nodes)
    nodes |= walk_neighbours(args.nodes)
    nodes = list(nodes)

    print(f"{len(nodes)} nodes registered: {', '.join(nodes)}")
    write_debug_file(f"Nodes: {len(nodes)}", os.getcwd(), "throughput.log")

    if len(nodes) == 0:
        raise RuntimeError("No nodes registered to connect to")

    print()

    # Retrieve throughput metrics
    metrics = retrieve_from_different_nodes_throughput(nodes, duration=1, N=5)

    # Store the results
    results["nodes"].append(len(nodes))
    results["put_mean"].append(metrics["put_mean"])
    results["put_std"].append(metrics["put_std"])
    results["get_mean"].append(metrics["get_mean"])
    results["get_std"].append(metrics["get_std"])

    print(f"Results for {len(nodes)} node(s):")
    print(f"  GET Success Rate: {metrics['success_mean']:.2f}")
    print(f"  Average PUT Requests: {metrics['put_mean']:.2f} ± {metrics['put_std']:.2f} requests/sec")
    print(f"  Average GET Requests: {metrics['get_mean']:.2f} ± {metrics['get_std']:.2f} requests/sec")
    print()

if __name__ == "__main__":
    parser = arg_parser()
    args = parser.parse_args()
    main(args)
