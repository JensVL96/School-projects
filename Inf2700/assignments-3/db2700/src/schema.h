...
extern tbl_p table_search(tbl_p t, char const* attr,
                          char const* op, int val, int type);
...
extern tbl_p table_natural_join(tbl_p left, tbl_p right, int key);
// extern tbl_p nested_loop_join(tbl_p left, tbl_p right);
// extern tbl_p block_nested_loop_join(tbl_p left, tbl_p right);
/** Perform a search operation on a table using binary search. */
extern int binary_search(record r , schema_p s, int offset, int (*op) (int, int), int val);
#endif
