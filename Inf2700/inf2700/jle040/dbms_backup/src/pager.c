...
static void put_fhandle_info ( pmsg_level level, fhandle_p fh )
{
  ...
  for ( size_t i = 0; i < NUM_PAGES; i++)
    if ( fh->blocks_in_mem[i] != NULL )
      append_msg (level,  " %d,", fh->blocks_in_mem[i]->blk_nr);
  append_msg (level,  "\n" );
}

/* forward declaration */
static fhandle_p get_tbl_file ( const char *fname );

void put_file_info ( pmsg_level level, const char *name )
{
...
void put_pager_info (pmsg_level level,  const char *msg )
{
  if ( msg == NULL)
    msg = "";
...
static void put_pqueue_info (pmsg_level level, pqueue_p q, char *which)
{
  if (q == NULL)
    {
...
{
  page_p p = (page_p) malloc (sizeof (page_struct));
  if ( p == NULL)
    {
...
{
  pqueue_p pq = (pqueue_p) malloc (sizeof (pqueue));
  pq->first = NULL;
  pq->last = NULL;
  pq->len = 0;
  return pq;
...
{
  pq_elm_p p = (pq_elm_p) malloc (sizeof (pq_elm));
  p->page = pg;
  pg->qelm = p;
  return p;
}
...
*/
static int find_fhandle_i ( const char *fname )
{
  for ( size_t i = 0, num_compared = 0; num_compared < num_file_handles; i++ )
    {
      if ( file_handles[i] != NULL )
	{
...
static fhandle_p make_fhandle ( const char *fname, int fd )
{
  fhandle_p fh = (fhandle_p) malloc (sizeof (file_handle_struct));
  fh->fname = (char *) malloc (strlen (fname) + 1);
...
/** Get the file handle if the file is already open.
If the file is not open, open it for reading and writing
and create a new handle for it.
If the file does not exist, create one and open it for reading
and writing.
*/
static fhandle_p get_or_open_tbl_file ( const char *fname )
{	
...
int file_num_blocks ( const char *fname )
{
  fhandle_p fh = get_or_open_tbl_file (fname);
...
static int close_tbl_file ( fhandle_p fhandle )
{
  if (fhandle == NULL)
    return -1;
  for ( size_t i = 0; i < NUM_PAGES; i++)
    {
      if ( fhandle->blocks_in_mem[i] == NULL)
	continue;
  ...
  for ( size_t i = 0; i < NUM_PAGES; i++)
    {
      if ( fh->blocks_in_mem[i] == NULL)
	continue;
      if ( fh->blocks_in_mem[i]->blk_nr == bnr )
	{
...
static void remove_blk_from_fhandle ( block_p b )
{
  for ( size_t i = 0; i < NUM_PAGES; i++)
	...
	{
	  b->fhandle->blocks_in_mem[i] = NULL;
	  return;
	}
    }
}
...
page_p get_page ( const char *fname, int blknr )
{
  fhandle_p fh = get_or_open_tbl_file (fname);
  block_p blk = NULL;
  ...
  if ( fh->current_block != NULL && blknr == fh->current_block->blk_nr )
  ...
  if ( blk == NULL )
    {
      blk = (block_p) malloc (sizeof (block_struct));
      blk->fhandle = fh;
      blk->blk_nr = blknr;
      if (pin ( blk ) == NULL)
	{
...
page_p get_page_for_append ( const char *fname )
{
...
int write_page (page_p p)
{
  ...
  p->dirty = 0;
  return write (fd, p->content, BLOCK_SIZE);
}
...
int page_valid_pos_for_put ( page_p p, int offset, int len )
{
  if (offset >= PAGE_HEADER_SIZE && offset <= p->free_pos
    ...
    return 1;
  /* put_msg (WARN, */
  /* 	   "page_valid_pos_for_put: offset %d out of range [%ld,%ld]\n", */
  /* 	   offset, PAGE_HEADER_SIZE, */
  /* 	   p->free_pos > (BLOCK_SIZE - len) ? p->free_pos : (BLOCK_SIZE - len)); */
  return 0;
}

/** Called after a successful put, so there is no need to check if
    current position is valid */
static int set_pos_after_put ( page_p p, int offset )
{
...
int page_put_str ( page_p p, char *str, int len )
{
...
int page_put_str_at ( page_p p, int offset, char *str, int len )
{
