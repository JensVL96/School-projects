 ...
 * time, generic pointer (void *) is used in many places and must be
 * cast to the right C types.  The types of a record's fields are
...
extern void put_record_info ( pmsg_level level, record r, schema_p s ); 
...
/** Open a database at @em new_db_dir.
    If @em dir is NULL, re-open a prviously openned database.
...
extern field_desc_p new_int_field ( const char *name );
/** Make an string field with name @em name and length @em len. */
extern field_desc_p new_str_field ( const char *name, int len );
...
extern void assign_int_field ( void *field_p, int int_val );
/** Assign a str record field. Note the data type of @em field_pp */
extern void assign_str_field ( void *field_p, char *str_val );
/** Fill a record with values. Example: fill_record (r, s, 1, "A string", 136)*/
extern int fill_record ( record r, schema_p s, ...);
/** Compare if two records have equal field values */
extern int equal_record ( record r1, record r2, schema_p s );
...
extern tbl_p table_search (tbl_p t, char *attr, char *op, int val, int type);
/** Make a new table as a result of project. */
extern tbl_p table_project ( tbl_p t, const char *dest_name,
