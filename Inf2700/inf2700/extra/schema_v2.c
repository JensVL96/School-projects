...
tbl_p joining_int_field(tbl_p left, tbl_p right, field_desc_p curr_left, field_desc_p curr_right, tbl_p result) 
{
  record left_rec = new_record(left->sch);
  record right_rec = new_record(right->sch);
  page_p left_pg = get_page_for_next_record(left->sch);

  int left_pos, left_rec_val;

  for ( ; left_pg != NULL; left_pg = get_page_for_next_record(left->sch)) {
    left_pos = page_current_pos(left_pg);
    left_rec_val = page_get_int_at(left_pg, left_pos + curr_left->offset /*Pid*/);
    printf("\nleft table position: %d, left table record value: %d\n", left_pos, left_rec_val);

    while (find_record_int_val(right_rec, right->sch, curr_right->offset /*Pid*/, int_equal, left_rec_val)) {
      append_record(right_rec, result->sch);
      append_record(left_rec + left_pos + curr_left->next->offset, result->sch);
    }
  }
  return result;
}
...
void check_same_fields(schema_p left, schema_p right, schema_p join_sch, char **name)
{
  int max_fields = left->num_fields + right->num_fields;
  // char ** save = malloc(max_fields * sizeof(char*));
  // char save[max_fields][10];

  field_desc_p curr_left = left->first;
  field_desc_p curr_right = right->first;

  for (int i = 0; i < left->num_fields; i++) {
    for (int j = 0; j < right->num_fields; j++) {
      if (strcmp(curr_left->name, curr_right->name) == 0) {
        strcpy(name[i], left->name);
      } 
  ...
  for (int k; k < max_fields; k++) {
    if (strcmp(curr_left->name, name[k]) != 0) {
      add_field(join_sch, curr_right);
    } else if (strcmp(curr_right->name, name[k]) != 0) {
      add_field(join_sch, curr_right);
    }
  }
}
...
tbl_p table_natural_join (tbl_p left, tbl_p right)
{
  ...
  schema_p join_sch;
  char join_name[30] = "natural_join__";
  join_sch = new_schema(join_name);

  int max_fields = left->sch->num_fields + right->sch->num_fields;
  // char save[max_fields][10];
  field_name_p f_name;
  f_name.save[max_fields][10];

  /* if (left == NULL) printf("\n\nnothing here\n\n");
  display_tbl_header(left);

  schema_p s = left->sch;
  record rec = new_record(s);
  int result = get_record(rec, s);
  //printf("\n\nresult: %d\n", result); 

  set_tbl_position(left, TBL_BEG);
  while (get_record(rec, left->sch)) {
    //printf("\n\nhere\n\n");
    for (size_t k = 0; curr_left != NULL; curr_left = curr_left->next, k++) {
      if (is_int_field (curr_left)) {
        printf("\n\nleft records: %20d\n\n", *(int *)rec[k]);
        //put_msg (FORCE, "%20d", *(int *)rec[k] );
      } else {
        printf("\n\nleft records: %20s\n\n", *(char *)rec[k]);
        //put_msg (FORCE, "%20s", (char *)rec[k] );
      }
    }
    put_msg (FORCE, "\n");
  }
  put_msg (FORCE, "\n");
  release_record (rec, left->sch); */


  check_same_fields(left->sch, right->sch, join_sch, f_name.save);

  assert(0);
  for (int s = 0; s < max_fields; s++) {
    printf("\nall of the same fields by name: %s\n", f_name.save[s]);
  }
  ...
  // Find if any column name (field) matches in the two tables
  for (int i = 0; i < left->sch->num_fields; i++) {
    printf("\n\nleft table field name: %s\n", curr_left->name);
    // add_field(join_tbl->sch, curr_left);
    for (int j = 0; j < right->sch->num_fields; j++) {
      printf("\n\nright table field name: %s\n", curr_right->name);
      if (strcmp(curr_left->name, curr_right->name) == 0) {
        //name exists join the tables
        // save data
        // delete same field from one

        if (is_int_field (curr_right)) {
          printf("\nit's an integer field\n");
          joining_int_field(left, right, curr_left, curr_right, join_sch->tbl);
        } else {
          printf("\nit's a string field\n");
          joining_str_field(left, right, curr_left, curr_right, join_sch->tbl);
        }
        curr_right = curr_right->next;
      } else {
        curr_right = curr_right->next;
    ...
    curr_left = curr_left->next;
    curr_right = right->sch->first;
  }


  // assert(0);
  // display_tbl_header(left);
  // table_display(join_tbl);
  // table_display(right);
  // table_display(join_tbl);

  // lagre informasjonene som er lik i f.eks. en liste
  // integer field?
  // alltid jobbe med forste kolonne
  // vet ikke jobbe med to offset
  // ved match tar info fra først -> legger inn, og andre -> legger inn, så slå disse sammen
  return join_sch->tbl;  
}
