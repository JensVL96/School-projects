...
schema_p finding_records_with_block(schema_p left, schema_p right, int l_offset, int r_offset, schema_p result) 
{
  ...
  for(int i = 0; i < file_num_blocks(left->name); i++) {
    ...
    for(int j = 0; j < file_num_blocks(right->name); j++) {
      ...
      for ( ; !eop(left_pg); left_pos = left_pos + left->len) {
        ...
        for ( ; !eop(right_pg); right_pos = right_pos + right->len) {
          printf("right pos: %d\n",  right_pos);
...
schema_p finding_records_with_tuple(schema_p left, schema_p right, int l_offset, int r_offset, schema_p result) 
{
  ...
  //  Loops though the pages on the left until page is empty
...
{
  //  Allocates memory to the field name storing array
  ...
  //  Goes though both schemas and adds the fields in common in the name array
  ...
  //  Resets the field pointers
  tmp_l = tmp_left->first;
  tmp_r = tmp_right->first;

  //  Loops until all fields are added to the joined/ result schema
  for ( ; tmp_l != NULL && tmp_r != NULL ; ) {
    //  Adds the field if it doesn't exist, then or if instead goes to the next field
    ...
    //  Adds the field in both exist cases then moves to next field
  ...
  //  Adds a NULL terminator at the end of the storing array
  name[counter] = NULL;
  return name;
}
...
tbl_p table_natural_join (tbl_p left, tbl_p right)
{
  ...
  //  Variable to determine tuple or block sorting
  int key = 1;

  //  Gives the new schema based on the sorting variation
  if (key == 0) {
    char join_name[30] = "natural_join_tuple";
    join_sch = new_schema(join_name);
  } else {
    char join_name[30] = "natural_join_block";
    join_sch = new_schema(join_name);
  }

  // Puts one of every field in the new joined schema
  char **save = combine_fields(left->sch, right->sch, join_sch);

  //  Searches until the field offset of the field in common is found
  for (int j = 0; save[j] != NULL; ) {
    if (strcmp(curr_right->name, save[j]) == 0 && strcmp(curr_right->name, save[j]) == 0) {
      if (is_int_field (curr_right)) {
        //  Sends the field offset so that the record data can be added
        if (key == 0) {
          finding_records_with_tuple(left->sch, right->sch, curr_left->offset, curr_right->offset, join_sch);
        } else {
          finding_records_with_block(left->sch, right->sch, curr_left->offset, curr_right->offset, join_sch);
        }
      }
      j++;
