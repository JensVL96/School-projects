 ...
 *
 * To process data stored in a table file, use @ref get_page "get_page()"
 * to get the page of a given block (if there is no page for the block yet,
 * associate a page with the block),
 * or use @ref get_page_for_append "get_page_for_append()" or
 * @ref get_next_page "get_next_page()" to get an appropriate page,
 * and then use @ref read_page "read_page()"
...
int num_file_handles;

/** Database buffer */
page_p pages[NUM_PAGES];

extern void put_file_info ( pmsg_level level, const char *fname );
extern void put_page_info ( pmsg_level level, page_p p); 
extern void put_block_info ( pmsg_level level, block_p b); 
extern void put_pager_info ( pmsg_level level, const char *msg ); 
...
extern int set_system_dir ( char *dir );
...
- otherwise,
  - make it managed in the file handle;
...
*/
extern page_p get_page ( const char *fname, int blknr );
/** Get the last block and move the current position to the end */
extern page_p get_page_for_append ( const char *fname );
/** Get the next page */
extern page_p get_next_page ( page_p p );
/** Set current position to the beginning */
void page_set_pos_begin ( page_p p );
/** Number of blocks in the file */
extern int file_num_blocks ( const char *fname );
/** Close the file */
extern int close_file ( const char *fname );
...
Returns -1 if fails (@em offset out of range)
*/
...
The current position is moved to the next value.
*/
extern int page_put_str ( page_p p, char *str, int len );
...
Returns -1 if fails (@em offset out of range).
*/
extern int page_put_str_at ( page_p p, int offset, char *str, int len );
