...
''' Class represents the main graphical interface for the Sudoku game '''
class SudokuGUI(tk.Tk):
    ...
    def __init__(self):
        ...
        self.sudoku_grid.fetch_new_board()

    # Create a grid of Entry widgets for the Sudoku board
    def create_widgets(self):
        # Frames for grid and padding
        ...
        # Placing the Entry widgets and creating frames and separators
        for i in range(9):
            for j in range(9):
                # Create Entry frame with highlighting
        ...
        solve_button = tk.Button(button_frame, text="Restart", command=self.sudoku_grid.restart)
        solve_button.grid(row=0, column=0, pady=10)

        clear_button = tk.Button(button_frame, text="Check", command=(self.sudoku_grid.check))
        clear_button.grid(row=1, column=0, pady=10)

        exit_button = tk.Button(button_frame, text="Solve", command=self.sudoku_grid.solve)
        exit_button.grid(row=2, column=0, pady=10)

        new_board_button = tk.Button(button_frame, text="New", command=self.sudoku_grid.fetch_new_board)
        new_board_button.grid(row=3, column=0, pady=10)

    # Handles arrow keypress events for navigation
    def on_arrow_keypress(self, delta_row, delta_col):
        focus_widget = self.focus_get()

        # Check if the focus is on Entry widgets
        if focus_widget in [entry for row in self.entries for entry in row]:
            current_row, current_col = self.get_entry_position(focus_widget)
    ...
    # Helper function to get the position of an entry in the grid
    def get_entry_position(self, entry_widget):
        for row, entries in enumerate(self.entries):
            if entry_widget in entries:
                return row, entries.index(entry_widget)
        return -1, -1

    # Handles the FocusOut event for an Entry widget
    def on_entry_focus_out(self, row, col):
        value = self.entries[row][col].get()
        self.sudoku_grid.board.nums[row][col].value = int(value)
        self.entries[row][col].config(bg='white')

''' Class manages the Sudoku board and its operations. '''
class SudokuGrid:
    # Constructor to initialize a SudokuGrid instance and bind the arrow keys
    def __init__(self, gui):
        self.gui = gui
        self.board = None

        # Bind arrow keys to the on_arrow_keypress function
        self.gui.bind('<Up>', lambda event: self.on_arrow_keypress(-1, 0))
        self.gui.bind('<Down>', lambda event: self.on_arrow_keypress(1, 0))
        self.gui.bind('<Left>', lambda event: self.on_arrow_keypress(0, -1))
        self.gui.bind('<Right>', lambda event: self.on_arrow_keypress(0, 1))

    # Fetches a new Sudoku board and updates the associated GUI. 
    def fetch_new_board(self):
        ...
        self.update_board_entries(self.board.nums)

    # Fetches a new Sudoku board and updates the associated GUI.
    def update_board_entries(self, grid):
        for i in range(9):
            for j in range(9):
                value = grid[i][j].value
                entry = self.gui.entries[i][j]
                entry.delete(0, tk.END)
                entry.insert(0, str(value))
                self.gui.entries[i][j].config(bg='white')

    # Retrieve values from the Entry widgets
    def solve(self, update=True):
        # Attempt to solve the board
        solution_found = self.solved.recursive_solve()

        if solution_found:
            if update:
                self.update_board_entries(self.solved.nums)
                messagebox.showinfo("Sudoku", "Solved it for you :P")
        else:
            messagebox.showinfo("Sudoku", "No solution found.")

    # Restarts the Sudoku board to its initial state.
    def restart(self):
        self.board = self.initial

        self.update_board_entries(self.board.nums)

    # Checks the current state of the Sudoku board.
    def check(self):    
        self.solve(False)
        ...
        for i in range(9):
            for j in range(9):  
                ...
                if current_value != solved_value and current_value != 0:
                    ...
                    self.gui.entries[i][j].config(bg='red')
                    flag = False
