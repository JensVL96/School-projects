...
class SudokuGUI(tk.Tk):
    def __init__(self):
        ...
        self.bind('<Up>', lambda event: self.on_arrow_keypress(-1, 0))
        self.bind('<Down>', lambda event: self.on_arrow_keypress(1, 0))
        self.bind('<Left>', lambda event: self.on_arrow_keypress(0, -1))
        self.bind('<Right>', lambda event: self.on_arrow_keypress(0, 1))
        ...
        for i in range(9):
            for j in range(9):
                ...
                entry = tk.Entry(
                    ...
                    justify='center', 
                    validate='key',
                    validatecommand=(self.register(lambda P: self.validate_entry(P, i, j)), '%P')
                )
    ...
    def validate_entry(self, P, row, col):
        if P.isdigit() and 0 <= int(P) <= 9:
            self.board.nums[row][col].value = int(P)
            return True
        elif not P:
            self.board.nums[row][col].value = 0
            return True
        else:
            return False
