...
class Flyer():
    ...
    def move_hoik(self, boids, hoiks, obstacles, time_passed):
        ...
        # Decay the number of boids eaten over time
        self.num_boids_eaten -= 0.1 * (time_passed / 1000)  # Reduce by 0.1 units per second

        # Ensure num_boids_eaten doesn't go below 0
        self.num_boids_eaten = max(0, self.num_boids_eaten)

        # Calculate the new size
        new_size = self.size * self.num_boids_eaten

        if new_size < HOIK_SIZE:
            new_size = HOIK_SIZE

         # Ensure size doesn't go below the initial value
        self.size = min(HOIK_SIZE, self.size)

        self.speVec += v2 + v3
        self.speVec = self.speVec.normalize() if self.speVec.length() != 0 else self.speVec
        self.pos += self.speVec * multiplier
        if self.num_boids_eaten > 0:
            self.speVec *= new_size
