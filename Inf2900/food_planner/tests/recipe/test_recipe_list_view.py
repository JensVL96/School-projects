...
class TestRecipeListView:
    ...
    def test_list_entries_displays_recipe_thumbnail(self, client, recipe_list):
        response = client.get(reverse("recipes"))
        content = response.content.decode()
        for recipe in response.context["recipe_list"]:
            img_src = f"""<img src="{recipe.image.url}" alt="{recipe}">"""
            assert img_src in content

    def test_list_entries_does_not_display_image_path(self, client, recipe_list):
        response = client.get(reverse("recipes"))
        content = response.content.decode()
        for recipe in response.context["recipe_list"]:
            assert recipe.image.path not in content
